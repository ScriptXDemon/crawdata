...
class HarvestStats:
    ...
    trap_skipped: int = 0          # URLs dropped by trap heuristics
...
def harvest(job: Job, fetcher: Fetcher, history: CrawlHistory | None = None,
    ...
    *on_fetch* is an optional callback receiving a dict with:
    ...
    # Per-host circuit breaker + one-shot transient requeue + trap state (sync parity
    # with the async engine — same policy outcomes, simpler mechanics).
    host_fails: dict[str, int] = {}
    dead_hosts: set[str] = set()
    retried: set[str] = set()
    query_variants: dict[str, int] = {}
    hard_fail_limit = int(os.environ.get("CRAWLER_HOST_HARD_FAILS", "3"))
    max_query_variants = int(os.environ.get("CRAWLER_MAX_QUERY_VARIANTS", "20"))
    ...
    try:
        while queue and _fetched.value < job.max_pages:
            ...
            if host in dead_hosts:
                stats.bump_error(errors.HOST_DOWN)
                continue
            if history is not None and history.is_gone(url):
                stats.bump_error(errors.GONE_SKIP)
                continue
            if errors.careful_off_peak_now(host):     # gov host, outside allowed hours
                stats.bump_error(errors.OFF_PEAK)
                continue
            ...
            if history is not None and errors.is_careful_host(host):   # audit gov/careful hosts
                try:
                    history.record_audit(
                        url=url, host=host, fetched_at=res.fetched_at or "",
                        ua=fetcher.user_agent,
                        robots_decision=("deny" if res.reason == errors.ROBOTS else "allow"),
                        status=res.status, reason=res.reason, careful=True)
                except Exception:
                    pass
            ...
            if res.error or res.status is None or res.status >= 400:
                reason = res.reason or errors.classify_failure(res.status, res.error) or errors.OTHER
                if history is not None:
                    history.record_failure(url, status=res.status, category=reason,
                                           failed_at=res.fetched_at or "")
                if reason in errors.HARD_FAIL:
                    host_fails[host] = host_fails.get(host, 0) + 1
                    if host_fails[host] >= hard_fail_limit:
                        dead_hosts.add(host)
                        log.warning("host_down host=%s fails=%d", host, host_fails[host])
                policy = (errors.policy_for_status(res.status)
                          if res.status and res.status >= 400 else None)
                transient = policy in (errors.RETRY_LATER, errors.COOLDOWN) or reason in errors.TRANSIENT
                if transient and url not in retried:
                    retried.add(url)             # one more attempt; FIFO tail = natural delay
                    queue.append((url, depth))
                    continue
                stats.bump_error(reason)
                log.info("fetch_fail url=%s status=%s reason=%s", url, res.status, reason)
                _notify("err")
                continue

            host_fails[host] = 0                  # a success clears the host's breaker count
            stats.fetched += 1
            _fetched.inc()
            _notify("ok")
            ...
            if res.is_html() and res.text_html:
                    ...
                    if depth < job.max_depth and _prune_seed_links(job, depth, res):
                        ...
                        for link in links:
                            ...
                            if qp.query:
                                ...
                                n = query_variants.get(key, 0) + 1
                                if n > max_query_variants:
                                    stats.trap_skipped += 1
                                    continue
                                query_variants[key] = n
                            seen.add(cl)
                ...
                except Exception as exc:            # malformed HTML crashing the parser
                    stats.bump_error(errors.PARSE_ERROR)
                    log.info("parse_error url=%s: %s", url, exc)
