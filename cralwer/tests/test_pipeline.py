...
def test_lt_news_sends_one_page_bundle_with_tags():
    h, c = _fresh()
    r = run_job(_job("LT_news"), c, SEED, h, MATCHER)
...
def test_tender_extracts_pdf_attachment_text():
    h, c = _fresh()
    r = run_job(_job("MOD_tender"), c, SEED, h, MATCHER)
    doc = r.documents[0]
    assert doc.stream == "tender"
...
def test_profile_detects_competitor_and_country():
    h, c = _fresh()
    r_nibe = run_job(_job("NIBE_profile"), c, SEED, h, MATCHER)
    r_solar = run_job(_job("SOLAR_profile"), c, SEED, h, MATCHER)
    nibe_doc = r_nibe.documents[0]
    assert nibe_doc.detected_competitor == "NIBE"
    assert "SIGSA" in nibe_doc.detected_unknown_companies or any(
        d.resolved_id == "SIGSA" for d in nibe_doc.entities_detected)
    solar_doc = r_solar.documents[0]
    assert solar_doc.detected_competitor == "SOLAR"
    assert "P_NAGASTRA123" in solar_doc.detected_products      # Solar's product, not a rival's


def test_non_english_translation():
    h, c = _fresh()
    r = run_job(_job("KNDS_news"), c, SEED, h, MATCHER)
    doc = r.documents[0]
...
def test_same_run_duplicate_content_emitted_once():
    ...
    r = run_job(job, c, SEED, h, MATCHER)
...
def test_recrawl_is_idempotent():
    h, c = _fresh()
    job = _job("LT_news")
    r1 = run_job(job, c, SEED, h, MATCHER)
    r2 = run_job(job, c, SEED, h, MATCHER)
    assert r1.sent > 0
...
def test_spec_keeps_all_nonjunk_images():
    h, c = _fresh()
    r = run_job(_job("CAESAR_spec"), c, SEED, h, MATCHER)
    doc = r.documents[0]
...
def test_samgov_tender_detects_country():
    h, c = _fresh()
    r = run_job(_job("SAMGOV_tender"), c, SEED, h, MATCHER)
    doc = r.documents[0]
    assert doc.stream == "tender"
    assert "USA" in doc.detected_countries
